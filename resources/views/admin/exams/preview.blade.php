...
<div class="p-6">
    <div class="mb-6">
        <div class="flex items-center justify-between mb-2">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('معاينة الامتحان') }}</h1>
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm mb-6">
            <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-4">{{ $exam->title }}</h2>
                
                @if($exam->description)
                    <div class="mb-4">
                        <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('وصف الامتحان') }}</h3>
                        <p class="text-gray-600 dark:text-gray-400">{!! nl2br(e($exam->description)) !!}</p>
                    </div>
                @endif

                @if($exam->instructions)
                    <div class="mb-4">
                        <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('تعليمات الامتحان') }}</h3>
                        <div class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                            <p class="text-blue-800 dark:text-blue-200">{!! nl2br(e($exam->instructions)) !!}</p>
                        </div>
                    </div>
                @endif
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    <div class="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
                        <div class="text-gray-500 dark:text-gray-400">{{ __('المدة') }}</div>
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
                        <div class="text-gray-500 dark:text-gray-400">{{ __('عدد الأسئلة') }}</div>
                        <div class="font-semibold text-gray-900 dark:text-white">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
                        <div class="text-gray-500 dark:text-gray-400">{{ __('إجمالي الدرجات') }}</div>
                        <div class="font-semibold text-gray-900 dark:text-white">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
                        <div class="text-gray-500 dark:text-gray-400">{{ __('درجة النجاح') }}</div>
                        <div class="font-semibold text-gray-900 dark:text-white">
                            {{ $exam->passing_marks }}%
                        </div>
                    </div>
                        ...
                        <div class="p-6">
                            ...
                            <div class="flex items-start justify-between mb-4">
                                <div class="flex items-center">
                                    <span class="bg-primary-100 text-primary-800 dark:bg-primary-900 dark:text-primary-200 text-sm font-medium px-3 py-1 rounded-full ml-4">
                                        {{ __('السؤال') }} {{ $index + 1 }}
                                    </span>
                                    @if($examQuestion->is_required)
                                        <span class="bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200 text-xs font-medium px-2 py-1 rounded-full ml-2">
                                            {{ __('إجباري') }}
                                        </span>
                                    @endif
                            ...
                            <!-- نص السؤال -->
                            <div class="mb-4">
                                <div class="text-gray-900 dark:text-white text-lg leading-relaxed">
                                ...
                                <div class="space-y-2">
                                    @foreach($examQuestion->question->options as $optionIndex => $option)
                                        <div class="flex items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                                            <div class="w-6 h-6 bg-white dark:bg-gray-600 border-2 border-gray-300 dark:border-gray-500 rounded-full flex items-center justify-center ml-3">
                            ...
                            @elseif($examQuestion->question->type === 'true_false')
                                <div class="space-y-2">
                                    <div class="flex items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                                        <div class="w-6 h-6 bg-white dark:bg-gray-600 border-2 border-gray-300 dark:border-gray-500 rounded-full flex items-center justify-center ml-3">
                                            <span class="text-sm font-medium text-gray-600 dark:text-gray-300">أ</span>
                                        </div>
                                        <span class="text-gray-900 dark:text-white">{{ __('صحيح') }}</span>
                                    </div>
                                    <div class="flex items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                                        <div class="w-6 h-6 bg-white dark:bg-gray-600 border-2 border-gray-300 dark:border-gray-500 rounded-full flex items-center justify-center ml-3">
                                            <span class="text-sm font-medium text-gray-600 dark:text-gray-300">ب</span>
                                        </div>
                                        <span class="text-gray-900 dark:text-white">{{ __('خطأ') }}</span>
                                    </div>
                                </div>
                            @elseif($examQuestion->question->type === 'fill_blank')
                                <div class="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                                    <div class="border-b-2 border-gray-400 dark:border-gray-500 pb-1 mb-2">
                                        <span class="text-gray-500 dark:text-gray-400 text-sm">{{ __('منطقة الإجابة') }}</span>
                                    </div>
                                </div>
                            @elseif(in_array($examQuestion->question->type, ['short_answer', 'essay']))
                                <div class="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                                    <div class="border border-gray-300 dark:border-gray-600 rounded-lg p-4 min-h-[100px] bg-white dark:bg-gray-800">
        ...
        <!-- ملخص الامتحان -->
        @if($exam->examQuestions->count() > 0)
            <div class="bg-primary-50 dark:bg-primary-900/20 rounded-xl p-6 mt-6">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-lg font-semibold text-primary-900 dark:text-primary-100 mb-2">
                            {{ __('ملخص الامتحان') }}
                        </h3>
                        <div class="text-primary-700 dark:text-primary-200 text-sm space-y-1">
