...
<div class="space-y-6">
        ...
        <div class="xl:col-span-3">
            <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg border border-gray-200 dark:border-gray-700">
                <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">معلومات التصنيف</h3>
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium
                        {{ $questionCategory->is_active ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200' }}">
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">اسم التصنيف</label>
                                <div class="font-semibold text-gray-900 dark:text-white">{{ $questionCategory->name }}</div>
                            </div>
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">السنة الدراسية</label>
                                <div class="text-gray-900 dark:text-white">{{ $questionCategory->academicYear->name ?? 'غير محدد' }}</div>
                            </div>
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">المادة الدراسية</label>
                                <div class="text-gray-900 dark:text-white">{{ $questionCategory->academicSubject->name ?? 'غير محدد' }}</div>
                            </div>
                        </div>
                        <div>
                            @if($questionCategory->parent)
                                <div class="mb-4">
                                    <label class="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">التصنيف الأب</label>
                                    <div class="text-gray-900 dark:text-white">{{ $questionCategory->parent->name }}</div>
                                </div>
                            @endif
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">المسار الكامل</label>
                                <div class="text-gray-900 dark:text-white">{{ $questionCategory->full_path }}</div>
                            </div>
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">الترتيب</label>
                                <div class="text-gray-900 dark:text-white">{{ $questionCategory->order }}</div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="space-y-4">
            <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                <div class="flex items-center">
                    ...
                    <div class="mr-3">
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $stats['total_questions'] }}</p>
                        <p class="text-sm text-gray-500 dark:text-gray-400">إجمالي الأسئلة</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                <div class="flex items-center">
                    ...
                    <div class="mr-3">
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $stats['direct_questions'] }}</p>
                        <p class="text-sm text-gray-500 dark:text-gray-400">أسئلة مباشرة</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                <div class="flex items-center">
                    ...
                    <div class="mr-3">
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $stats['subcategories'] }}</p>
                        <p class="text-sm text-gray-500 dark:text-gray-400">تصنيفات فرعية</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    @if($questionCategory->children->count() > 0)
        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg border border-gray-200 dark:border-gray-700">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                <h4 class="text-lg font-semibold text-gray-900 dark:text-white">التصنيفات الفرعية</h4>
            </div>
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach($questionCategory->children as $subCategory)
                        <div class="p-4 border border-gray-200 dark:border-gray-700 rounded-lg hover:shadow-md transition-shadow">
                            <div class="flex items-center justify-between mb-2">
                                <h5 class="font-medium text-gray-900 dark:text-white">{{ $subCategory->name }}</h5>
                                <span class="text-sm text-gray-500 dark:text-gray-400">{{ $subCategory->questions->count() }} سؤال</span>
                            </div>
                            @if($subCategory->description)
                                <p class="text-sm text-gray-600 dark:text-gray-400 mb-3">{{ Str::limit($subCategory->description, 80) }}</p>
                            @endif
    ...
    @if($questionCategory->questions->count() > 0)
        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg border border-gray-200 dark:border-gray-700">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                    <h4 class="text-lg font-semibold text-gray-900 dark:text-white">الأسئلة في هذا التصنيف</h4>
    ...
    @if($stats['by_type']->count() > 0 || $stats['by_difficulty']->count() > 0)
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            @if($stats['by_type']->count() > 0)
                <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg border border-gray-200 dark:border-gray-700">
                    <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                        <h4 class="text-lg font-semibold text-gray-900 dark:text-white">توزيع الأسئلة حسب النوع</h4>
                    </div>
                    <div class="p-6">
                        <div class="space-y-3">
            ...
            @if($stats['by_difficulty']->count() > 0)
                <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg border border-gray-200 dark:border-gray-700">
                    <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                        <h4 class="text-lg font-semibold text-gray-900 dark:text-white">توزيع الأسئلة حسب الصعوبة</h4>
                    </div>
                    <div class="p-6">
                        <div class="space-y-3">
