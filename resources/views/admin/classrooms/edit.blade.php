        ...
        <div class="bg-white dark:bg-gray-800 shadow-lg rounded-lg mb-6">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">تعديل الفصل: {{ $classroom->name }}</h1>
                        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">تحديث بيانات الفصل الدراسي</p>
                    </div>
        ...
        <div class="mt-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="p-5">
                    <div class="flex items-center">
                        ...
                        <div class="mr-5 w-0 flex-1">
                            <dl>
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 truncate">عدد الطلاب</dt>
                                <dd class="text-lg font-bold text-gray-900 dark:text-white">{{ $classroom->students()->count() }}</dd>
                            </dl>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-lg rounded-lg">
                <div class="p-5">
                    <div class="flex items-center">
                        ...
                        <div class="mr-5 w-0 flex-1">
                            <dl>
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 truncate">تاريخ الإنشاء</dt>
                                <dd class="text-lg font-bold text-gray-900 dark:text-white">{{ $classroom->created_at->format('Y-m-d') }}</dd>
                            </dl>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-lg rounded-lg">
                <div class="p-5">
                    <div class="flex items-center">
                        <div class="flex-shrink-0">
                            <div class="w-8 h-8 bg-{{ $classroom->is_active ? 'green' : 'red' }}-100 dark:bg-{{ $classroom->is_active ? 'green' : 'red' }}-900 rounded-full flex items-center justify-center">
                                <i class="fas fa-{{ $classroom->is_active ? 'check' : 'times' }} text-{{ $classroom->is_active ? 'green' : 'red' }}-600 dark:text-{{ $classroom->is_active ? 'green' : 'red' }}-400"></i>
                            </div>
                        </div>
                        <div class="mr-5 w-0 flex-1">
                            <dl>
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 truncate">الحالة</dt>
                                <dd class="text-lg font-bold text-{{ $classroom->is_active ? 'green' : 'red' }}-600 dark:text-{{ $classroom->is_active ? 'green' : 'red' }}-400">
