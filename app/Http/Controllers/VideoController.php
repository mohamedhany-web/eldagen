    ...
    private function canAccessLesson($user, $lesson)
    {
        ...
        if ($user->isStudent()) {
            return $user->courseEnrollments()
                ->where('advanced_course_id', $lesson->advanced_course_id)
                ->where('status', 'active')
                ->exists();
        }

        // ولي الأمر يمكنه الوصول إذا كان ابنه مسجل
        if ($user->isParent()) {
            return $user->children()
                ->whereHas('courseEnrollments', function ($query) use ($lesson) {
                    $query->where('advanced_course_id', $lesson->advanced_course_id)->where('status', 'active');
                })
                ->exists();
        }
